$(function() {
	...
	var $issuedTable = $('#IssuedBookTable');
	if ($issuedTable.length) {
		var jsonUrl = window.contextRoot + '/json/data/cart/issued/books';
		console.log(jsonUrl);
		...
		$booksTable.DataTable({
				...
				},
				{
					...
					mRender: function(data, type, row) {
						...
						str += '<a href="'
							...
							+ '/manage/'
							+ data
							+ '/book" class="btn btn-primary"><span class="glyphicon glyphicon-pencil"></span></a> &#160;';
			...
			initComplete: function() {
				...
				api.$('.switch input[type="checkbox"]').on('change', function() {
					var dText = (this.checked) ? 'You want to activate the Book?' : 'You want to de-activate the Book?';
					...
					bootbox.confirm({
						...
						callback: function(confirmed) {
							if (confirmed) {
								$.ajax({
									type: 'GET',
									url: window.contextRoot + '/manage/book/' + checkbox.prop('value') + '/activation',
									timeout: 100000,
	...
	// validating the book form element	
		...
		if (countField.val() !== originalCount) {
			...
			if (countField.val() != 1) {
				...
				bootbox.alert({
					size: 'medium',
					title: 'Error',
					message: 'Book Count should be 1 !'
				});
			}
			else {
